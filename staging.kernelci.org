...
cmd_api_pipeline() {
    compose_files="docker-compose.yaml docker-compose-kcidb.yaml"

    echo "Stopping pipeline containers"
    cd checkout/kernelci-pipeline
    for compose in $compose_files; do
        [ -f $compose ] || continue
        echo "Stopping containers with compose file: $compose"
    ...
    for compose in $compose_files; do
        [ -f $compose ] || continue
        echo "Starting containers with compose file: $compose"
        REQUIREMENTS=requirements-dev.txt docker-compose -f $compose build
